    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        ...
        if let image = info[UIImagePickerController.InfoKey.editedImage] as? UIImage, let imageData = image.pngData() {
                ...
                case .success(let urlString):
                    ...
                    DatabaseManager.shared.sendMessage(to: conversationId, otherUserEmail: strongSelf.otherUserEmail, name: name, newMessage: message) { success in
                        if success {
                            ...
                            print("faild to send photo.")
                        }
                    }
                    
                case .failure(let error):
                    print("msg photo upload error: \(error)")
                }
            }
            ...
            StorageManager.shared.uploadMessageVideo(with: videoUrl, fileName: fileName) { [weak self] result in
                ...
                case .failure(let error):
                    print("msg video upload error: \(error)")
                }
            }
        }
    }
...
extension ChatViewController: MessagesDataSource, MessagesLayoutDelegate, MessagesDisplayDelegate, MessageCellDelegate {
    ...
    func didTapImage(in cell: MessageCollectionViewCell) {
        ...
        case .photo(let media):
            guard let imgUrl = media.url else { return }
            let vc = PhotoViewerViewController(with: imgUrl)
